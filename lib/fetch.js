 ...
 */
export const get = (key) => ModelCache.get(key);
 ...
 * @return {boolean}
 */
export const exists = (key) => !!ModelCache.get(key);
 ...
 *
 *   * ajaxType {string} - request type (GET|POST|PUT|DELETE)
 *   * attriModels {object|array} - attributes to pass the Model, or models to pass the collection
 *   * fetch {boolean} - whether fetch the model after creation
 *   * fetchData {object} - data to pass into the fetch method
 ...
 *    Collection instance
 */
export const fetch = (key, Model, options={}) => {
  var model = ModelCache.get(key),
      addToLoadingCache,
      _promise;

  options = {
    attriModels: (Model && Model.prototype instanceof Backbone.Collection) ? [] : {},
    fetch: true,
    fetchData: {},
    forceFetch: false,
    ajaxType: 'GET',
  ...
  if (!loadingCache[key]) {
    _promise = new Promise((resolve, reject) => {
      if (!model || options.forceFetch) {
        model = model || new Model(options.attriModels, options.options || {});
