  ...
  (Component) => {
    ...
    class DataCarrier extends React.Component {
      ...
      componentWillUnmount() {
        ...
        this._unattachModelListeners.call(this);
      ...
      _fetchResources(resources, props=this.props) {
        return fetchResources(resources, props, {
          ...
          if (this._isMounted) {
            // attaches listeners on all resources (removes any existing first)
            this._attachModelListeners();
          }
        });
      }
      ...
      _attachModelListeners() {
        var modelsToAttach = this._generateResources()
            .map(([, config]) => getModelFromCache(config))
            .filter(Boolean);

        // unattach any previously-attached listeners first
        this._unattachModelListeners();

        // whenever there may be a change in the data, trigger a React update.
        modelsToAttach.forEach((model) => model.onUpdate(
          this._onModelEvent,
          this.listeningContext || this
        ));
      ...
      /**
       * Removes all model event listeners from a component. This happens automatically
       * on componentWillUnmount, but also before any listeners are manually attached.
       */
      _unattachModelListeners() {
        // Ensure that we clean up any dangling references when the component is destroyed.
        (this._attachedModels || []).forEach((model) => model.offUpdate(
          this._onModelEvent,
          this.listeningContext || this
        ));
          ...
          <ErrorBoundary>
            <Component
              {...(Component.prototype || {}).isReactComponent ? {
                // allows for our listeners to call forceUpdate in this context, so that even
                // PureComponents get updated
                ref: (dataChild) => this.listeningContext = dataChild
              } : {}}
      ...
      fetchResources(resourcesToFetch, props, {
        ...
        if (isMountedRef.current) {
          ...
          listenedModels.forEach((model) => {
            ...
            model.onUpdate(forceUpdate, model);
          });
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      resources.map(([, config]) => getModelFromCache(config))
          ...
          .forEach((model) => model.offUpdate(forceUpdate, model));
    };
  }, []);
