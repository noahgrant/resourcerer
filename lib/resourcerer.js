...
export const useResources = (getResources, props) => {
  ...
  var cachedModelsSinceLastEffect = useRef({});
  var cachedResources = pendingResources.concat(loadedResources)
  ...
  return {
    ...
    // helpers
    isOrWillBeLoading: () => {
      var _resourcesToUpdate = resourcesToUpdate.filter(withoutNoncritical).filter(withoutPrefetch);

      // here we only look at resources to fetch, not loaded ones
      return !hasLoaded(criticalLoadingStates) || !!partitionResources(
        _resourcesToUpdate,
        // 'next' loading states
        buildResourcesLoadingState(_resourcesToUpdate, props)
      )[1].length;
    },
    refetch: (fn) => {
