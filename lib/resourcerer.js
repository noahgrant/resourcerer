...
export const useResources = (getResources, props) => {
  var [resourceState, setResourceState] = useState({}),
      ...
      [
        {loadingStates, requestStatuses, errorResponses, hasInitiallyLoaded},
        loaderDispatch
      ] = useReducer(loaderReducer, {
  ...
  useEffect(() => {
    ...
    if (resourcesToUpdate.length) {
      ...
      fetchResources(resourcesToFetch, props, {
        ...
        onRequestFailure: (response, [name, config]) => {
          ReactDOM.unstable_batchedUpdates(() => {
...
/**
 * @param {string} name - string name of the resource
 * @return {string} name of the error response property
 */
function getErrorResponseName(name) {
  return `${name}ErrorResponse`;
}
...
function loaderReducer(
  ...
  var {name, status, json, resources} = payload;
  ...
  switch (type) {
    ...
    case LoadingStates.LOADED: {
      ...
      return {
        ...
        errorResponses: {...errorResponses, [getErrorResponseName(name)]: json},
