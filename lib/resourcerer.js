...
import {hasErrored, hasLoaded, isLoading, noOp, pick} from './utils';
  ...
  useEffect(() => {
    ...
    if (resourcesToUpdate.length) {
      ...
      fetchResources(resourcesToUpdate, props, {
        ...
        onRequestComplete: ([name, config]) => props.refetches?.includes(name) ?
          setResourceState((state) => ({
            ...state,
            refetches: props.refetches.filter((key) => key !== name)
          })) : null
      }).then(() => {
...
function fetchResources(resources, props, {
  ...
  return Promise.all(
    ...
    resources.map(([name, config]) => {
      ...
      ).then(() => onRequestComplete([name, config]));
    })
  );
}
