...
const overlayBackground = {backgroundColor: 'rgba(255, 255, 255, 0.6)'};
  ...
  (Component) => {
    class LoadingOverlay extends React.Component {
      ...
      componentWillReceiveProps(nextProps) {
        if (!this.state.hasInitiallyLoaded && nextProps.hasLoaded) {
      ...
      render() {
        ...
        return (
          <div style={{position: 'relative', zIndex: 1000}}>
            <div style={{...overlayStyle, ...overlayBackground}}>
              {this.props.isLoading && !options.noLoader ? (
                <span style={overlayStyle}><Loader /></span>
              ) : null}
            </div>
            {this.state.hasInitiallyLoaded || this.props.hasErrored ? (
              <NoUpdateIfLoading forwardRef={(child) => this.child = child} {...this.props} />
            ) : null}
          </div>
        );
      }
    ...
    class NoUpdateIfLoading extends React.Component {
      ...
      render() {
        var {forwardRef, ...rest} = this.props;
